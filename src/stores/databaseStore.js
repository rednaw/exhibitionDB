...
export const queries = readable(null, function start(set) {
  set({
    'Artic': 'select aic_start_at as start_date, aic_end_at as end_date, title as exhibition, image_id, id from exhibitions',
    'Metropolitan': 'select * from MetObjects',
    'Rijksmuseum': 'select * from collection',
    'ExhibitionDB': 'select e.Anno_Esposizione, e.Titolo_Esposizione, u.Luogo_Espositivo, c.Città_visione, c.Nazione from Ubicazioni u, Esposizioni e, Città c where e.ID_Ubicazioni = u.ID_Ubicazioni and u.ID_Città = c.ID_Città'
  })
    ...
    async ($title, $query) => {
      if ($title && $query) { // TODO: figure out better way to only update when both are triggered.
        ...
        if (!keys.includes(key)) {
          const result = await runQueryImpl($title, $query)
          if (result) {
